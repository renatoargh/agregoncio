...
module.exports = {
  async createAggregator (aggregator, lambdaArn) {
    ...
    assert(aggregator.name, 'aggregator should contain an `name` property')
    ...
    console.log('> creating aggregation topic')
    const aggregatorTopicArn = await sns.createTopic(aggregator.name)

    console.log('> inserting aggregation record')
    await dynamodb.putItem({
      'Item': ddt.wrap({
        topic: aggregatorTopicArn,
    ...
    await sns.subscribeTo(aggregator.events, lambdaArn, aggregatorTopicArn)
  },
